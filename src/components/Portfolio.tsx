...
const Portfolio = () => {
  ...
  return <section id="portfolio" className="section bg-[#E6E6FA]/30 relative overflow-hidden">
      ...
      <div className="container relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-8">
          {projects.map(project => <HoverCard key={project.id}>
              <HoverCardTrigger asChild>
                <button onClick={() => openProject(project)} className="group flex flex-col items-center gap-3 transition-transform hover:scale-105">
              ...
              <HoverCardContent className="w-80 bg-white/95 border border-[#D0E8FF] text-[#2C2C2C] shadow-md">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2">
                    {project.videos.length > 0 && <FileVideo className="h-4 w-4 text-[#1E90FF]" />}
                    {project.images.length > 0 && <ImageIcon className="h-4 w-4 text-[#1E90FF]" />}
                  </div>
                </div>
              </HoverCardContent>
            </HoverCard>)}
        ...
        <Dialog open={!!selectedProject} onOpenChange={() => closeProject()}>
          {selectedProject && <DialogContent className="bg-white/95 border-[#D0E8FF] p-0 max-w-4xl w-[90vw] max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                ...
                {selectedProject.videos.length > 0 && <div className="mb-6">
                    <div className="flex items-center gap-2 mb-3">
                      <FileVideo className="h-5 w-5 text-[#1E90FF]" />
                      <h3 className="text-lg font-semibold text-[#2C2C2C]">Vidéos</h3>
                    </div>
                    <div className="grid grid-cols-1 gap-4">
                      {selectedProject.videos.map((video, index) => <div key={index} className="overflow-hidden rounded-lg shadow-md">
                          <video src={video} className="w-full h-auto" controls autoPlay muted loop />
                        </div>)}
                    </div>
                  </div>}
                ...
                {selectedProject.images.length > 0 && <div>
                    ...
                    <div className="relative">
                      <div className="overflow-hidden rounded-lg shadow-md">
                        <img src={selectedProject.images[currentImageIndex]} alt={`${selectedProject.artist} - Image ${currentImageIndex + 1}`} className="w-full h-auto" />
                      </div>
                      
                      {selectedProject.images.length > 1 && <div className="absolute inset-0 flex items-center justify-between px-4">
                          <button onClick={e => {
                    e.stopPropagation();
                    prevImage();
                    ...
                    {selectedProject.images.length > 1 && <div className="flex justify-center mt-4">
                        {selectedProject.images.map((_, index) => <button key={index} onClick={e => {
                  e.stopPropagation();
                  setCurrentImageIndex(index);
                }} className={`w-3 h-3 mx-1 rounded-full transition-colors ${currentImageIndex === index ? "bg-[#1E90FF]" : "bg-[#D0E8FF]"}`} />)}
                      </div>}
                  </div>}
              </div>
            </DialogContent>}
